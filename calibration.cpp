        ...
        vector<float>& perViewErrors )
{
    ...
    for( i = 0; i < (int)rvecs.size(); i++ )
    {
        ...
        {
            omnidir::projectPoints(Mat(objectPoints[i]), imagePoints2, rvecs[i], tvecs[i],
                                   cameraMatrix, xi.at<double>(0, 0), distCoeffs);
        }
...
static bool runCalibration( vector<vector<Point2f> > imagePoints,
                    ...
                    int flags, Mat& cameraMatrix, Mat& xi, Mat& distCoeffs,
                    ...
                    double& totalAvgErr)
{
    ...
    else
    {
        ...
        try
        {
            //std::vector<std::vector<Vec3d>> tmp(objectPoints.size());
            //for (size_t i = 0; i < objectPoints.size(); ++i)
            //{
            //    tmp[i].resize(objectPoints[i].size());
            //    std::transform(objectPoints[i].begin(), objectPoints[i].end(), tmp[i].begin(), [](const Point3f& p){return Vec3d(p.x, p.y, p.z);});
            //}
            //std::vector<std::vector<Vec2d>> tmp2(imagePoints.size());
            //for (size_t i = 0; i < imagePoints.size(); ++i)
            //{
            //    tmp2[i].resize(imagePoints[i].size());
            //    std::transform(imagePoints[i].begin(), imagePoints[i].end(), tmp2[i].begin(), [](const Point2f &p)
            //                   { return Vec2d(p.x, p.y); });
            //}
            rms = omnidir::calibrate(objectPoints, imagePoints, imageSize, cameraMatrix, xi, distCoeffs, rvecs, tvecs,
                flags | omnidir::CALIB_FIX_GAMMA | omnidir::CALIB_FIX_SKEW,
    ...
    totalAvgErr = computeReprojectionErrors(objectPoints, imagePoints,
                rvecs, tvecs, cameraMatrix, xi, distCoeffs, reprojErrs);
...
static void saveCameraParams( const string& filename,
                       ...
                       const Mat& cameraMatrix, cv::Mat xi, const Mat& distCoeffs,
                       const vector<Mat>& rvecs, const vector<Mat>& tvecs,
...
static bool runAndSave(const string& outputFilename,
                ...
                float grid_width, bool release_object,
                float aspectRatio, int flags, Mat& cameraMatrix, Mat& xi,
                Mat& distCoeffs, bool writeExtrinsics, bool writePoints, bool writeGrid )
{
    ...
    bool ok = runCalibration(imagePoints, imageSize, boardSize, patternType, squareSize,
                   aspectRatio, grid_width, release_object, flags, cameraMatrix, xi, distCoeffs,
    ...
    if( ok )
        saveCameraParams( outputFilename, imageSize,
                         ...
                         flags, cameraMatrix, xi, distCoeffs,
...
int main( int argc, char** argv )
{
    ...
    Mat cameraMatrix, xi, distCoeffs;
    ...
    for(i = 0;;i++)
    {
       ...
       // improve the found corners' coordinate accuracy
        ...
        Mat alma;
        cv::resize(view, alma, cv::Size(), 0.25, 0.25);
        imshow("Image View", alma);
